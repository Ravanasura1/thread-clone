...
const Actions = ({ post }) => {
  ...
  const [liked, setLiked] = useState(post?.likes?.includes(user?._id) ?? false); // Provide a default value for liked
  ...
  const handleReply = async () => {
    if (!user)
      return showToast(
        "Error",
        "You must logged in to reply to a post",
        "error"
      );

    if (!reply) {
      showToast("Error", "Please enter a reply text", "error");
      return;
    }
